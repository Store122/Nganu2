...
const wit = moment.tz('Asia/Jayapura').format('HH : mm : ss')
       ...
       kahfz.createMessage = async (jidnya, kontennya, optionnya) => {
		...
		const troli = {
                         ...
                         "surface": 200, 
                         "message": ` 𝙱𝙾𝚃 𝙼𝙳 ${pushname}! `, 
                         "orderTitle": 'i Mlbb 𝙱𝙾𝚃 𝙼𝙳!', 
                         "sellerJid": '6283811034750@s.whatsapp.net'
                    } 
                          } 
                              } 
        ...
        "hydratedButtons": [
{
            "urlButton": {
              "displayText": "𝙷𝙾𝚂𝚃𝙸𝙽𝙶",
              "url": "https://replit.com/@fahrigans"
            }
          },
          {
            "quickReplyButton": {
        ...
        "hydratedButtons": [
          ...
          },
          {
                                       callButton: {
                                            ...
                                            phoneNumber: '6283811034750s.whatsapp.net'
                                        }
                                    }, 
...
case 'bcgc': case 'bc': case 'broadcast': {
                ...
                for (let i of anu) {
                    await sleep(1500)
                    await kahfz.copyNForward(i, kahfz.cMod(m.chat, cc, /bc|broadcast|bcgc/i.test(cck) ? cck : ` ʙᴏᴛ ʙʀᴏᴀᴅᴄᴀꜱᴛ \n\n ${cck}`), true).catch(_ => _)
                }
...
case 'doaniat':
              ...
              teksnyaa = `NIAT SHOLAT ${query}\n${asw.result}[ *NIAT SHOLAT BY MLBB - BOT* ]` 
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'owner': case 'creator': {
                let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    ...
                    + 'FN:catzy\n' // full name
                    + 'ORG:ilham - skhyzi;\n' // the organization of the contact
                    + 'TEL;type=CELL;type=VOICE;waid=6283811034750:+6283811034750\n' // WhatsApp ID + phone number
                    + 'END:VCARD'
                kahfz.sendMessage(m.chat, { contacts: { displayName: 'owner mlbb - bot md', contacts: [{ vcard }] } }, { quoted: troli })
            }
            break
			case 'kick': {
